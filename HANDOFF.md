...
- **FIXED (commit 6ccfe07):** scroll-wheel + PTT (side-click) were dead on the **Home screen** because `show('home')` was clearing the selection model (`selItems=[]`), so `moveSel()`/`activate()` had nothing to act on. Home now re-renders + re-registers selection on `show('home')`. Also added a native `wheel` event fallback (some firmware emits `wheel` instead of the synthetic `scrollUp`/`scrollDown`), a 120ms move debounce (a double synthetic event couldn't skip two items), and a `#debug` URL flag that logs every `scrollUp/scrollDown/sideClick/longPressStart/wheel/keydown` to the console for diagnosing which events the device actually fires. Defined the missing `selfRide()` alias (quick-tap a saved route now jumps straight to the QR).
